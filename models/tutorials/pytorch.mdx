...
This tutorial uses a single set of hyperparameters. In practice, most machine learning workflows involve testing multiple configurations to find better-performing models.

W&B Sweeps lets you automate hyperparameter searches and compare results across runs. You define the parameters to explore, choose a search strategy, and let W&B manage the experiments.
...
This example shows the smallest set of changes required to track a PyTorch training run with Weights & Biases.

Use this pattern if you already have a training loop and want to add experiment tracking with minimal refactoring.
...
* Creates a tracked W&B run with wandb.init.

* Logs hyperparameters for reproducibility and comparison.

* Records training metrics during the loop with run.log.

* Optionally tracks gradients with run.watch.

* Optionally saves model artifacts with run.save.
